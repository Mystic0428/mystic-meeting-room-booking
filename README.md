...
```bash
./mvnw test       # 47 個測試(unit + repository + controller + 整合/併發),需 Docker(Testcontainers)
...
> 註:原本的「改用 JWT 授權」已於加分項實作完成(見第 3 節「認證」與 §12③)。以下為現階段仍想改進的三點:

1. **佔用狀態單一來源**:目前佔用中狀態在 Java(`OCCUPYING_STATUSES`)與 DB constraint 雙寫,理想上由單一來源衍生,避免改一邊忘另一邊。
2. **API 文件自動化**:導入 OpenAPI 讓清單由註解產生,並補上題目加分的營業時間 / 單次時長上下限 validation。
