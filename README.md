...
- 驗證:`curl http://localhost:8080/api/rooms` → 回傳 5 間種子會議室。
...
所有路徑前綴 `/api`。錯誤一律由 `GlobalExceptionHandler` 統一回傳 `ApiError`(400 / 403 / 404 / 409 / 500)。
...
Schema 由 Flyway migration 管理(`src/main/resources/db/migration`,V1–V8),`ddl-auto: validate` 只驗證不自動建表。
...
| `APPROVED` | ✅ 佔用 | 題目規則 10 |
| `CANCEL_REQUESTED` | ✅ 佔用 | 題目未定義,**自訂**(理由見第 12 節「需求釐清 ①」) |
| `REJECTED` | ❌ 不佔用 | 題目規則 8 |
...
- Spring Security / JWT 身分驗證(加分;目前操作者 id 由 request 帶入)
...
1. **授權**:改由 JWT / `SecurityContext` 取得當前使用者,不再信任 request body 的 id。
2. **佔用狀態單一來源**:目前佔用中狀態在 Java(`OCCUPYING_STATUSES`)與 DB constraint 雙寫,理想上由單一來源衍生,避免改一邊忘另一邊。
3. **API 文件自動化**:導入 OpenAPI 讓清單由註解產生,並補上題目加分的營業時間 / 單次時長上下限 validation。
...
- **無身分驗證機制**:`userId` / `reviewerId` 由 request body 傳入,Service 內做授權(退回比對是否本人、審核檢查是否 `REVIEWER`/`ADMIN`)。**已知限制**:信任呼叫端;正式環境應改由 JWT / `SecurityContext` 取得當前使用者。
